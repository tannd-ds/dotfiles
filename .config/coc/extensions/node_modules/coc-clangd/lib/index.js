  ...
  constructor(client, context) {
    ...
    context.subscriptions.push(client.onDidChangeState(({ newState }) => {
      if (newState === import_coc3.State.Running) {
        const notification = new import_vscode_languageserver_protocol.NotificationType("textDocument/semanticHighlighting");
    ...
    for (const [line, tokens] of lines.entries()) {
      for (const token of tokens) {
        if (token.kind === "InactiveCode") {
          skipped.push(import_vscode_languageserver_protocol.Range.create(line, token.character, line, token.character + token.length));
        } else {
          symbols.push({
            id: 0,
            kind: token.kind,
            ranges: [import_vscode_languageserver_protocol.Range.create(line, token.character, line, token.character + token.length)],
            parentKind: "Unknown",
            storage: "None"
          });
        }
...
var UI = class {
  ...
  get clangdPath() {
    return coc.workspace.expand(this.config.get("path", ""));
  }
  set clangdPath(p) {
...
var MemoryUsageFeature = class {
  ...
  initialize(capabilities) {
    if ("memoryUsageProvider" in capabilities) {
      this.ctx.subscriptions.push(import_coc7.commands.registerCommand("clangd.memoryUsage", async () => {
...
var ASTFeature = class {
  ...
  initialize(capabilities) {
    if ("astProvider" in capabilities && typeof import_coc8.window["createTreeView"] === "function") {
      ...
      this.ctx.subscriptions.push(tree, adapter.onDidChangeTreeData(() => {
        if (adapter.hasRoot()) {
          tree.reveal(null);
        }
        ...
        if (!this.ctx.client)
          return;
        let range = null;
        const { document: document2, position } = await import_coc8.workspace.getCurrentState();
        const mode = await import_coc8.workspace.nvim.call("visualmode");
        if (mode)
          range = await import_coc8.workspace.getSelectedRange(mode, import_coc8.workspace.getDocument(document2.uri));
        if (!range)
        ...
        adapter.setRoot(item, import_coc8.Uri.parse(document2.uri), winid);
        tree.show();
      }));
    }
  }
...
async function activate2(context) {
  ...
  context.subscriptions.push(fileStatus, import_coc9.commands.registerCommand("clangd.switchSourceHeader", switchSourceHeader(ctx)), import_coc9.commands.registerCommand("clangd.symbolInfo", symbolInfo(ctx)), import_coc9.commands.registerCommand("clangd.userConfig", userConfig), import_coc9.commands.registerCommand("clangd.projectConfig", projectConfig), ctx.client.onDidChangeState((e) => {
    var _a2;
